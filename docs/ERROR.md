...
## Silence gate trips on retry — "We couldn't hear you" after a failed attempt

**Context:** `src/hooks/usePronunciation.ts` — Hold-to-Record on the lesson screen, second and subsequent presses within the same word.
**Symptom:** First attempt records and scores normally. After a wrong/low-score verdict, tapping Retry and holding record again surfaces "We couldn't hear you — try holding the button while speaking clearly" even though the child is speaking clearly. Silence gate skips the API call so no POST is ever made.
**Cause:** `useAudioRecorderState(recorder, 80)` continuously polls native metering at 80 ms — even when not recording. After `recorder.stop()`, the metering value stays pinned at the previous session's last reading (often the user's voice ≈ 0.5–0.7 normalized). On retry, `recorder.record()` is fired without `await`, then `setIsRecording(true)` flips immediately. The metering effect ingested those carry-over frames as the first frames of the new session. Calibration (3 frames / 240 ms) trained the noise floor on stale loud values and clamped it to the upper bound `0.35`. Path A (single peak ≥ 0.35) couldn't fire because no fresh speech frames had arrived yet, and Path B (sustained > floor) required the user to sustain >0.35 — much harder than the default 0.20. Net: silence gate trips on every retry until the recorder stops being polled. The single-frame `lastMeteringValueRef` dedupe was insufficient — it only filtered the first matching tick.
**Fix:** Two complementary gates in `src/hooks/usePronunciation.ts`:
1. `recordingStartedAtRef` is stamped immediately after `recorder.record()` returns. The metering `useEffect` drops every frame whose `Date.now() - recordingStartedAtRef.current < WARMUP_MS` (120 ms) — these frames are guaranteed to be from the previous session.
2. `lastMeteringValueRef.current` is now reset to `null` in `startRecording` alongside the other refs, so cross-session dedupe state cannot leak.
